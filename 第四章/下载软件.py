...
def downloadfile(url,name):
    ...
    resp1.close()
    print('download over')
async def downloadts(url,session,i,name,episode,list):
    attmept=0
    for attmept in range(5):
        try:
            async with session.get(url,headers=headers) as resp:
                async with aiofiles.open(f'online/第四章/{name}/{episode}/{i}.ts',mode='wb') as f:
                    content=await resp.read()
                    await f.write(content)
                print(f'{url.split('/')[-1]}的第{attmept+1}次下载成功')
                return
        except asyncio.TimeoutError:
            if attmept<=2:
                print(f'连接超时，尝试第{attmept+2}次连接')
            else :
                print(f'第{episode}集中第{i}个片段下载失败')
                list.append(f'第{episode}集中第{i}个片段下载失败')
                break
async def aio_downloads(name,episode,errorlist):
    tasks=[]
    i=0
        ...
        async with aiofiles.open(f'{s}.m3u8',mode='r',encoding='utf-8') as f:
            async for line in f:
                if line.startswith('#'):
                    ...
                    line=line.strip()
                task=asyncio.create_task(downloadts(line,session,i,name,episode,errorlist))
                tasks.append(task)
                i+=1
...
def want():
    ...
    return url,name
def showerror(name,list):
    if len(list)==0:
        print('下载非常成功')
    else:
        with open(f'online/第四章/{name}/errorlist.txt',mode='w') as f:
            for error in list:
                print(error)
                f.writelines(error)
def main():
